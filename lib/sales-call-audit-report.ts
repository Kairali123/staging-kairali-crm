...
// The daily agent-wise call audit report, in the one shape both consumers use:
// `app/api/sales-call-audit/email-data` (the on-screen template) and
// `app/api/cron/sales-call-audit-daily-email` (the 09:00 IST dispatch).
//
// This logic used to live inline in the email-data route handler. It was lifted
// here verbatim — same queries, same fallback order, same rounding — so the cron
// mails exactly the figures the page shows, rather than a second implementation
...
export interface SalesCallAuditReport {
  source: "daily_sales_reports_log_fms" | "kairali_sales_metric_bot_for_ho"
  data: SalesCallAuditEmailData
}
...
export async function buildSalesCallAuditReport(
  requestedDate?: string | null
): Promise<SalesCallAuditReport> {
  const pool = await getPool()
  let selectedDate = requestedDate || null
