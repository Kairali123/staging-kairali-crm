...
export function exportSalesHTML(report:DailySalesReport,scope:string){
 ...
 const cx=100,cy=100,R=90,r=56
 let currentOffset=0
 const donutSlices=totalSales>0?contributors.map((c,i)=>{
  const fraction=c.sales/totalSales
  if(fraction<=0)return ''
  const color=palette[i%palette.length]
  if(fraction>=0.9999){
   return `<path d="M ${cx} ${cy-R} A ${R} ${R} 0 1 1 ${cx} ${cy+R} L ${cx} ${cy+r} A ${r} ${r} 0 1 0 ${cx} ${cy-r} Z" fill="${color}"/><path d="M ${cx} ${cy+R} A ${R} ${R} 0 1 1 ${cx} ${cy-R} L ${cx} ${cy-r} A ${r} ${r} 0 1 0 ${cx} ${cy+r} Z" fill="${color}"/>`
  }
  const a0=2*Math.PI*currentOffset-Math.PI/2
  currentOffset+=fraction
  const a1=2*Math.PI*currentOffset-Math.PI/2
  const x1=cx+R*Math.cos(a0),y1=cy+R*Math.sin(a0)
  const x2=cx+R*Math.cos(a1),y2=cy+R*Math.sin(a1)
  const x3=cx+r*Math.cos(a1),y3=cy+r*Math.sin(a1)
  const x4=cx+r*Math.cos(a0),y4=cy+r*Math.sin(a0)
  const largeArc=fraction>0.5?1:0
  return `<path d="M ${x1.toFixed(2)} ${y1.toFixed(2)} A ${R} ${R} 0 ${largeArc} 1 ${x2.toFixed(2)} ${y2.toFixed(2)} L ${x3.toFixed(2)} ${y3.toFixed(2)} A ${r} ${r} 0 ${largeArc} 0 ${x4.toFixed(2)} ${y4.toFixed(2)} Z" fill="${color}" stroke="#ffffff" stroke-width="1.5"/>`
 }).join(''):''
 const donutChartHTML=totalSales>0?`<div style="width:200px;margin:0 auto;text-align:center">
<svg viewBox="0 0 200 200" width="190" height="190" style="display:block;margin:0 auto">
${donutSlices}
<circle cx="${cx}" cy="${cy}" r="${r-1}" fill="#ffffff"/>
<text x="${cx}" y="86" text-anchor="middle" font-family="Arial,sans-serif" font-size="8" font-weight="700" fill="#64748b" letter-spacing="0.8">ALL CONTRIBUTORS</text>
<text x="${cx}" y="105" text-anchor="middle" font-family="Georgia,serif" font-size="14" font-weight="bold" fill="#1e305b">${esc(money(totalSales))}</text>
<text x="${cx}" y="121" text-anchor="middle" font-family="Arial,sans-serif" font-size="9" fill="#64748b">${contributors.length} contributor${contributors.length===1?'':'s'}</text>
</svg>
</div>`:`<div style="width:200px;margin:0 auto;text-align:center"><svg viewBox="0 0 200 200" width="190" height="190" style="display:block;margin:0 auto"><circle cx="100" cy="100" r="90" fill="#e7ecf7"/><circle cx="100" cy="100" r="55" fill="#ffffff"/><text x="100" y="105" text-anchor="middle" font-family="Arial,sans-serif" font-size="12" fill="#64748b">No sales</text></svg></div>`
 ...
 const callingHTML=`<section>
  <div style="display:flex;justify-content:space-between;align-items:flex-end;gap:16px;flex-wrap:wrap;margin-bottom:14px">
   <div><h2 style="margin:0;font-size:20px;color:#1e305b">Calling overview</h2></div>
   <div style="text-align:right;font-size:11px;line-height:1.7;color:#64748b">
    <span>Pending totals: ${esc(scope==='ALL'?'all companies':scope)} · ${esc(pendingModeLabel)} · ${esc(pendingDateLabel)}</span><br>
    <span>Calls done: Live sheet · ${esc(callsDoneDateLabel)} · employee totals cover all companies</span>
   </div>
  </div>
  ${callingCardsHTML}
