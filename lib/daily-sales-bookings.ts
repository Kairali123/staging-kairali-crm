...
export async function bookingAmounts(bookings:BookingAggregate[],cancellations:BookingAggregate[]){
 ...
 const rows=await Promise.all([...bookings.map(r=>({r,cancel:false})),...cancellations.map(r=>({r,cancel:true}))].map(async({r,cancel})=>({company:'KTAHV',agent:r.agent,verified:cancel?0:await inr(r),cancelled:cancel?await inr(r):0,cancelledCount:cancel?r.records:0,unverified:0,records:r.records,conversions:cancel?0:r.records})))
 return rows
}
