...
export async function POST(req:NextRequest){
 ...
 try{
  ...
  const config=await transaction(s=>{
   ...
   s.runs.push({id:input.requestId,triggerId:c.id,startedAt:new Date().toISOString(),status:'Sending'});return c
  })
  requestId=input.requestId
  const messageId=await sendReport(config,input.to,input.date,image)
  await transaction(s=>{const run=s.runs.find(r=>r.id===requestId)!;run.status='Accepted';run.messageId=messageId})
  return NextResponse.json({status:'Accepted',message:'Redlava accepted the message. Delivery is not yet confirmed.'},{headers})
 }catch(e){
  if(requestId)await transaction(s=>{const run=s.runs.find(r=>r.id===requestId);if(run)run.status='Unknown'}).catch(()=>{})
