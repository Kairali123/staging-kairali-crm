...
// Client selects a stage (1/2/3) via the body, never an arbitrary action.
const STAGE_ACTIONS: Record<number, string> = {
    1: "accountStatusUpdate1",
    2: "accountStatusUpdate2",
...
export async function POST(req: NextRequest) {
    ...
    if (!hasAdminRole(session, "lower") && !hasServerActionPermission(session, "ktahvPage", "accountsVerify")) {
        return NextResponse.json({ success: false, error: "Insufficient permissions" }, { status: 403 });
    }

    const sharedSecret = process.env.GAS_SHARED_SECRET;
    if (!sharedSecret) {
        return NextResponse.json({ success: false, error: "Accounts verification service is not configured" }, { status: 503 });
    }
    ...
    try {
        ...
        if (err?.name === "AbortError") {
            return NextResponse.json({ success: false, error: "Accounts verification service timed out" }, { status: 504 });
        }
        return NextResponse.json({ success: false, error: "Accounts verification service is unreachable" }, { status: 502 });
    } finally {
        clearTimeout(timer);
    }
}
