...
// Fixed, server-owned deployment/action: no stage selection, client cannot
// control the upstream URL or action.
...
export async function POST(req: NextRequest) {
    ...
    if (!sharedSecret) {
        return NextResponse.json({ success: false, error: "Checkout verification service is not configured" }, { status: 503 });
    }
    ...
    try {
        const upstream = await fetch(GAS_CHECKOUT_URL, {
            method: "POST",
        ...
        if (err?.name === "AbortError") {
            return NextResponse.json({ success: false, error: "Checkout verification service timed out" }, { status: 504 });
        }
        return NextResponse.json({ success: false, error: "Checkout verification service is unreachable" }, { status: 502 });
    } finally {
        clearTimeout(timer);
    }
}
