...
export async function POST(req: NextRequest) {
    ...
    if (!sharedSecret) {
        return NextResponse.json({ success: false, error: "Approval service is not configured" }, { status: 503 });
    }
    ...
    try {
        const upstream = await fetch(GAS_APPROVAL_URL, {
            method: "POST",
            headers: { "Content-Type": "text/plain;charset=utf-8" },
            body: JSON.stringify({
                ...body,
                uploadedBy: access.actorName,
                uploadedByEmail: String(access.user?.email || "").trim(),
                sharedSecret,
            }),
        ...
        if (err?.name === "AbortError") {
            return NextResponse.json({ success: false, error: "Approval service timed out" }, { status: 504 });
        }
        return NextResponse.json({ success: false, error: "Approval service is unreachable" }, { status: 502 });
    } finally {
        clearTimeout(timer);
    }
}
