...
export async function GET(req:NextRequest) {
 ...
 // Management-wide aggregates remain restricted to existing administrator access.
 if(!hasAdminRole(user,'lower') && !hasPermission(user,'all')) return NextResponse.json({error:'Administrator access is required for company-wide reporting.'},{status:403,headers})
 let params
