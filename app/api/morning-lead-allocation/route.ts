...
} from '@/lib/morning-lead-allocation'
...
export async function POST(req: NextRequest) {
  ...
  const validActions = ['START', 'OWNER_CHANGE', 'AVAILABILITY', 'CONFIRM', 'VERIFIED_NO_CHANGE']
  if (
    !input ||
    !validActions.includes(input.action) ||
    [input.key, input.previousOwner, input.newOwner, input.reason].some(
      (item) => item && (typeof item !== 'string' || item.length > 300),
    ) ||
  ...
  if (!actor)
    return NextResponse.json({ error: 'Signed-in user has no auditable identity' }, { status: 403, headers })
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : ''
    if (/Source row transferred, but the master log/.test(message))
      return NextResponse.json({ error: message, code: 'TRANSFER_LOG_INCOMPLETE' }, { status: 502, headers })
    if (/changed since refresh|Choose an available|Enter a reason|Person is not in Config|Resolve unassigned|Start the morning check|already started|Source unavailable|multiple sheets|view-only|Remarks cell|partly updated|not found in current snapshot/.test(message))
