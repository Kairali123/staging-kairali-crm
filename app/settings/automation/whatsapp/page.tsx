...
export default function WhatsAppTriggers(){
 ...
 async function refresh(){setBusy(true);setError('');try{const state=await json('/api/whatsapp-trigger-config');setSchedulerReady(state.schedulerReady);setProviderReady(state.providerReady);const s=await json('/api/whatsapp-trigger-config/templates');setTemplates(s.templates);setCheckedAt(s.checkedAt);setProviderReady(true);setNotice('Template status refreshed from Redlava')}catch(e){setTemplates([]);setCheckedAt('');setError(e instanceof Error?e.message:'Could not verify templates')}finally{setBusy(false)}}
