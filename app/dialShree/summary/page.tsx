...
import {
    ...
    ArrowUpDown, ArrowUp, ArrowDown, AlertCircle, PhoneCall,
} from "lucide-react";
import {
...
// ── Import the two drilldown modals ───────────────────────────────────────────
...
const GroupCells = React.memo(({
    ...
    if (isGrand) return (
        <>
            ...
            <td className="px-2 py-2 text-center whitespace-nowrap" style={{ background: "rgba(120,53,15,0.15)", borderLeft: "1px solid rgba(255,255,255,0.08)" }}>
        ...
        <div>
                    ...
                    <table className="min-w-full" style={{ borderCollapse: "collapse" }}>
                                        ...
                                        <tr className="cursor-pointer hover:bg-slate-50 transition-colors border-b-2 border-slate-200"
                                            ...
                                            <td className="sticky left-0 z-10 px-4 py-3 whitespace-nowrap"
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-xs font-bold text-slate-800">{dg.displayDate || dg.date}</span>
                                                    <span className="text-[10px] font-medium text-slate-400">({dg.sources.length} sources)</span>
                                                </div>
                                            </td>
                                            {GROUPS.map(g => (
                                                <GroupCells key={g.key}
                                                    ...
                                                    tatTotal={g.getTAT ? g.getTAT(t) : 0}
                                                    tatH={g.getTATH ? g.getTATH(t) : 0}
                                                    tatM={g.getTATM ? g.getTATM(t) : 0}
                                                    tatL={g.getTATL ? g.getTATL(t) : 0}
                                                    isDateRow showTAT={g.showTAT}
                                                />
                                            ))}
                                        </tr>
                                        {expanded && dg.sources.map((s, si) => (
                                            <tr key={`${dg.date}-${s.source}-${si}`}
                                                className="hover:bg-blue-50/40 transition-colors border-b border-slate-100 bg-white">
                                                <td className="sticky left-0 z-10 pl-8 pr-4 py-2 whitespace-nowrap bg-white border-r border-slate-200">
                                                    <div className="flex items-center gap-2">
                                                        <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: s.color || "#64748b" }} />
                                                        <div className="flex flex-col">
                                                            <span className="text-xs font-semibold text-slate-800">{s.source}</span>
                                                ...
                                                {GROUPS.map(g => (
                                                    <GroupCells key={g.key}
                                                        total={g.getTotal(s as any)} H={g.getH(s as any)} M={g.getM(s as any)} L={g.getL(s as any)}
                                                        denominator={g.getDenominator(s as any, grand)}
                                                        tatTotal={g.getTAT ? g.getTAT(s as any) : 0}
                                                        tatH={g.getTATH ? g.getTATH(s as any) : 0}
                                                        tatM={g.getTATM ? g.getTATM(s as any) : 0}
                                                        tatL={g.getTATL ? g.getTATL(s as any) : 0}
                                                        showTAT={g.showTAT}
                                                        onCellClick={(intent: string) => onCellInteract({
                                                            date: dg.date,
                                                            company: s.company,
                                                            src: s.source,
                                                            intent,
                                                            statusParam: g.modalStatus,
                                                        })}
                                                    />
                                                ))}
                                            </tr>
                        ...
                        <tfoot>
                            <tr style={{ backgroundColor: "#0f172a" }}>
                                <td className="sticky left-0 z-20 px-4 py-3 text-left whitespace-nowrap"
                                    style={{ backgroundColor: "#0f172a", borderRight: "2px solid rgba(255,255,255,0.15)" }}>
                                ...
                                {GROUPS.map(g => (
                                    <GroupCells key={g.key}
                                        ...
                                        tatTotal={g.getTAT ? g.getTAT(grand) : 0}
                                        tatH={g.getTATH ? g.getTATH(grand) : 0}
                                        tatM={g.getTATM ? g.getTATM(grand) : 0}
                                        tatL={g.getTATL ? g.getTATL(grand) : 0}
                                        isGrand showTAT={g.showTAT}
                                    />
                                ))}
                            </tr>
                        </tfoot>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 border-t border-slate-200 bg-slate-50">
                <span className="text-xs font-medium text-slate-500">
                    Showing <strong className="text-slate-700">{totalDates === 0 ? 0 : startIdx + 1}</strong> to <strong className="text-slate-700">{Math.min(startIdx + rowsPerPage, totalDates)}</strong> of <strong className="text-slate-700">{totalDates}</strong> dates
                </span>
                <div className="flex items-center gap-1.5">
                    <button onClick={() => goToPage(1)} disabled={safePage <= 1} className="px-2 py-1 rounded border border-slate-300 bg-white text-xs font-semibold text-slate-600 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-100 transition">«</button>
                    <button onClick={() => goToPage(safePage - 1)} disabled={safePage <= 1} className="px-2.5 py-1 rounded border border-slate-300 bg-white text-xs font-semibold text-slate-600 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-100 transition">‹ Prev</button>
                    <span className="px-2 text-xs font-semibold text-slate-600">Page {safePage} of {totalPages}</span>
                    <button onClick={() => goToPage(safePage + 1)} disabled={safePage >= totalPages} className="px-2.5 py-1 rounded border border-slate-300 bg-white text-xs font-semibold text-slate-600 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-100 transition">Next ›</button>
                    <button onClick={() => goToPage(totalPages)} disabled={safePage >= totalPages} className="px-2 py-1 rounded border border-slate-300 bg-white text-xs font-semibold text-slate-600 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-100 transition">»</button>
                </div>
                <div className="flex items-center gap-2">
                    <span className="text-xs text-slate-500 font-medium">Rows:</span>
                    <select aria-label="Rows per page" value={rowsPerPage} onChange={e => handleRowsChange(Number(e.target.value))} className="h-7 rounded border border-slate-300 bg-white text-slate-700 text-xs px-2 font-semibold focus:outline-none">
...
// ─────────────────────────────────────────────────────────────────────────────
//  Main DialShree Summary Page
// ─────────────────────────────────────────────────────────────────────────────

export default function DialShreeSummaryReportPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/dialshree/summary?${params.toString()}`);
    ...
    // ── Handle cell click → open modal with exact filtered leads ───────────────
        ...
        try {
            ...
            if (matchedDay) {
                const matchedSources = matchedDay.sources.filter(s => {
                    ...
                    const sMatch = params.src === "ALL" || s.source.toUpperCase() === params.src.toUpperCase();
            ...
            if (params.statusParam === "SENT") {
                if (params.intent !== "ALL") {
                    sentRows = sentRows.filter(r => {
                        ...
                        if (params.intent === "HIGH") return intent.includes("high");
                        if (params.intent === "MEDIUM") return intent.includes("medium");
                        return !intent.includes("high") && !intent.includes("medium");
                    });
                }
            } else {
            ...
            const typeStr = `DialShree | Date: ${displayDate} | Company: ${params.company} | Source: ${params.src} | Status: ${statusLabel} | Intent: ${intentLabel}`;
    ...
    // ── Client-side filtering & Aggregations ──────────────────────────────────
    const stats = useMemo(() => {
    ...
    if (loading && rawDateGroups.length === 0) {
        return (
            <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 flex flex-col items-center justify-center min-h-[60vh]">
                ...
                <p className="mt-4 text-base font-bold text-indigo-600 animate-pulse">Fetching latest DialShree Summary...</p>
            </div>
        );
    }
    ...
    return (
        <div className="font-sans bg-[#f0f2f8] min-h-full text-slate-800">
            ...
            {/* ── Banner ── */}
            <div style={{ background: "linear-gradient(110deg,#0f172a 0%,#1e293b 35%,#312e81 70%,#4338ca 100%)", padding: "16px 20px", display: "flex", alignItems: "center", position: "relative", overflow: "hidden", flexWrap: "wrap", gap: 12 }}>
                <div style={{ position: "absolute", right: -60, top: -60, width: 220, height: 220, borderRadius: "50%", background: "rgba(255,255,255,.05)" }} />
                <div style={{ width: 40, height: 40, borderRadius: 10, background: "rgba(255,255,255,.15)", display: "flex", alignItems: "center", justifyContent: "center", marginRight: 12, flexShrink: 0, color: "#fff" }}>
                    <PhoneCall className="w-5 h-5 text-white" />
                </div>
                <div style={{ flex: 1, minWidth: 180 }}>
                    <div style={{ fontSize: 18, fontWeight: 800, color: "#fff", letterSpacing: "-.3px", lineHeight: 1.2 }}>DialShree Dialer Outreach &amp; Lead Summary</div>
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,.6)", marginTop: 3 }}>DialShree Dialer · Entity-wise Outbound Performance &amp; Disposition Insights</div>
                </div>
            ...
            <div className="mt-3 mx-3 sm:mx-5">
                <div className="rounded-xl border border-slate-200 bg-white shadow-md">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 px-5 py-4 bg-gradient-to-r from-indigo-100 via-white to-blue-100 border-b border-slate-200 rounded-t-xl">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-indigo-600 via-blue-600 to-indigo-700 flex items-center justify-center shadow-md border border-indigo-700/30">
                                <Search className="w-5 h-5 text-white" />
                            </div>
                            <div>
                                <h3 className="text-sm font-semibold text-slate-900">Filters</h3>
                                <p className="text-xs text-slate-500">Refine DialShree results by date, company, source and priority</p>
                            </div>
                        </div>
                        <Button variant="outline" size="sm" onClick={clearFilters} className="bg-white border-slate-300 text-slate-700 font-medium hover:bg-indigo-50">Clear Filters</Button>
                    </div>
            ...
            <div className="mx-3 sm:mx-5 mt-3 mb-5">
                <div className="bg-white border border-slate-200 rounded-xl shadow-md overflow-hidden">
                    <div className="flex flex-wrap items-center justify-between gap-3 px-4 sm:px-5 py-4 border-b border-slate-100">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl bg-indigo-600 flex items-center justify-center shadow-md flex-shrink-0 text-white">
                                {icons.dashboard(18)}
                            </div>
                            <div>
                                <h3 className="text-[14px] font-bold text-slate-900 leading-tight">DialShree Breakdown</h3>
                                <p className="text-[11px] text-slate-400 mt-0.5">Date-wise Outreach &amp; Call Performance — High · Medium · Low quality across all metrics</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
