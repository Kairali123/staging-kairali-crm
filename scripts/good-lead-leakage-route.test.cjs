...
const m=new Module(__filename,module);m.require=id=>{
 ...
 if(id==='@/lib/authz')return {getSessionUser:()=>user}
...
;(async()=>{const req={nextUrl:new URL('http://localhost/api/good-lead-leakage')};assert.equal((await m.exports.GET(req)).status,401);user={role:'sales'};assert.equal((await m.exports.GET(req)).status,403);user={role:'admin'};assert.equal((await m.exports.GET(req)).status,403);user={role:'super_admin'};invalid=true;assert.equal((await m.exports.GET(req)).status,400);assert.deepEqual(events,[])
