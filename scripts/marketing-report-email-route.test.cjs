...
function harness({user={id:'one',role:'super_admin'},origin='http://localhost',valid=true,configured=true}={}){
 ...
 vm.runInNewContext(compiled,{exports,require:name=>{
  ...
  if(name==='@/lib/authz')return{getSessionUser:()=>user};
...
test('email route rejects unsigned, unauthorized, cross-origin and tampered requests without sending',async()=>{
 for(const [opts,status]of [[{user:null},401],[{user:{role:'admin'}},403],[{user:{role:'sales'}},403],[{origin:'https://other.example'},403],[{valid:false},400],[{configured:false},503]]){const h=harness(opts);assert.equal((await h.post()).status,status);assert.equal(h.mails.length,0)}
});
